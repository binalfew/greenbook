...
import { createSSOConfiguration } from "~/services/sso.server";
import { requirePermission } from "~/utils/auth/require-auth.server";
import {
  IDP_INSTRUCTIONS,
  SSO_PROTOCOL_OPTIONS,
  SSO_PROVIDER_OPTIONS,
} from "~/utils/constants/sso";
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  await createSSOConfiguration(submission.value, { userId: user.id });
...
export default function NewSSOConfigPage({ loaderData, actionData, params }: Route.ComponentProps) {
  const { t } = useTranslation("sso");
  const { t: tc } = useTranslation("common");
  const { callbackUrl, roles } = loaderData;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-foreground text-2xl font-bold">{t("addProvider")}</h2>
        <p className="text-muted-foreground mt-1 text-sm">{t("addProviderSubtitle")}</p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t("setupGuidesSubtitle")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-1">
          {Object.entries(IDP_INSTRUCTIONS).map(([key, guide]) => (
            <details key={key} className="group rounded-md border">
              <summary className="hover:bg-muted/50 cursor-pointer list-none px-3 py-2 text-sm font-medium">
                {guide.title}
              </summary>
