...
import { createRoleSchema } from "~/utils/schemas/security";
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  const role = await createRole(submission.value, ctx);

  return redirect(`/${params.tenant}/settings/security/roles/${role.id}`);
}

const SCOPE_OPTIONS = [
  { value: "TENANT", label: "Tenant — scoped to this organization" },
  { value: "GLOBAL", label: "Global — applies across all tenants" },
  { value: "EVENT", label: "Event — scoped to a specific event" },
];

export default function NewRolePage({ actionData, params }: Route.ComponentProps) {
  const { form, fields } = useForm(createRoleSchema, { lastResult: actionData });
  const backTo = `/${params.tenant}/settings/security/roles`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-foreground text-2xl font-bold">New role</h2>
