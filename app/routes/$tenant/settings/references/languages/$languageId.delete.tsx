...
import { deleteLanguage, getLanguage, ReferenceDataError } from "~/services/reference-data.server";
import { requirePermission } from "~/utils/auth/require-auth.server";
import { buildServiceContext } from "~/utils/request-context.server";
import type { Route } from "./+types/$languageId.delete";
...
export default function DeleteLanguage({ loaderData, params }: Route.ComponentProps) {
  const { t } = useTranslation("references");
  const { t: tc } = useTranslation("common");
  const base = `/${params.tenant}/settings/references/languages`;
  const d = loaderData.language;
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="divide-y">
          ...
          {d.nativeName && <InfoRow label={t("nativeName")}>{d.nativeName}</InfoRow>}
        </CardContent>
      </Card>
