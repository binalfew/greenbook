...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const result = await listLanguagesPaginated(tenantId, {
    page,
    pageSize: 25,
    where: { ...(search && { search }) },
  });
...
export default function LanguagesIndex({ loaderData, params }: Route.ComponentProps) {
  ...
  const columns: ColumnDef<Row>[] = [
    {
      ...
      cell: (row) => (
        <div className="min-w-0">
          ...
          {row.nativeName && row.nativeName !== row.name && (
            <p className="text-muted-foreground text-xs">{row.nativeName}</p>
          )}
        </div>
      ),
    },
