...
import { listOrganizationTypesPaginated } from "~/services/reference-data.server";
import { requirePermission } from "~/utils/auth/require-auth.server";
import type { Route } from "./+types/index";

export const handle = { breadcrumb: "Organization types" };
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const result = await listOrganizationTypesPaginated(tenantId, {
    page,
    pageSize: 25,
    where: { ...(search && { search }) },
  });
...
export default function OrganizationTypesIndex({ loaderData, params }: Route.ComponentProps) {
  const { t } = useTranslation("references");
  const { t: tc } = useTranslation("common");
  const base = `/${params.tenant}/settings/references/organization-types`;
