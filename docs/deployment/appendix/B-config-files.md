...
| [`app-vm/greenbook.conf`](app-vm/greenbook.conf)                           | `/etc/nginx/sites-available/greenbook.conf`       | Per-app server block (HTTP-only, source-pinned to DMZ) — first of N (greenbook today; future apps onboard via [06 §6.5](../06-app-vm-nginx-tls.md#65-adding-a-second-app-on-the-app-vm)) |
