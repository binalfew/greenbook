...
>
> The pattern above (ship `prisma/` + `prisma.config.ts` + `dotenv` into runtime, fetch prisma CLI from npm at deploy time) works but accumulates concerns onto the runtime image that aren't needed for serving traffic. The cleaner long-term shape is to tag the build stage during `docker build` as a second image (`greenbook-builder:$VERSION`) and run all migrations from THAT, not from the runtime image. The build stage already has the prisma CLI, the schema, the config, and dotenv — nothing extra to copy or skip. The runtime image goes back to lean: drop the `prisma/` and `prisma.config.ts` COPY lines and keep dotenv as a devDep again. Trade-off: Path B (tarball ship) doubles the bandwidth per deploy (~+700 MB for the builder image). Defer until you do enough migrations that this matters; until then the current shape is fine.
