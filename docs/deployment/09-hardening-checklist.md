...
- [§9.1 Operating system](#91-operating-system)
- [§9.2 PostgreSQL](#92-postgresql)
- [§9.3 Docker and the app container](#93-docker-and-the-app-container)
- [§9.4 Nginx and TLS](#94-nginx-and-tls)
- [§9.5 Secrets](#95-secrets)
- [§9.6 Observability](#96-observability)
...
- **Ubuntu 24.04 LTS, fully patched. No outstanding reboot required.**
  ...
  Set in: [01-pre-flight.md §1.1](01-pre-flight.md), [08-day-2-operations.md §8.5](08-day-2-operations.md)
...
- **`unattended-upgrades` is enabled and applied a security update in the last week.**
  ...
  Set in: [01-pre-flight.md §1.4](01-pre-flight.md)
...
- **SSH: password auth OFF, root login OFF, key-only.**
  ...
  Set in: [01-pre-flight.md §1.5](01-pre-flight.md)
...
- **fail2ban active and the sshd jail is watching.**
  ...
  Set in: [01-pre-flight.md §1.7](01-pre-flight.md)
...
- **No shared logins. Each operator has a personal sudo account; the only "system" account is `deployer` (app VM only, no sudo).**
  ...
  Set in: [01-pre-flight.md §1.8](01-pre-flight.md)
...
- **Authentication is `scram-sha-256` for every network client. No `md5`, no `trust` over TCP.**
  ...
  Set in: [02-db-vm-setup.md §2.2](02-db-vm-setup.md), [02-db-vm-setup.md §2.5](02-db-vm-setup.md)
...
- **`listen_addresses` is `localhost,<internal IP>` — never `*` or `0.0.0.0`.**
  ...
  Set in: [02-db-vm-setup.md §2.4](02-db-vm-setup.md)
...
- **`pg_hba.conf` allows ONLY the app VM IP on `/32`, no broader subnet.**
  ...
  Set in: [02-db-vm-setup.md §2.5](02-db-vm-setup.md)
...
- **UFW allows port 5432 ONLY from the app VM IP, not from "Anywhere".**
  ...
  Set in: [02-db-vm-setup.md §2.7](02-db-vm-setup.md)
...
- **Database passwords are ≥ 24 chars and generated by `openssl rand -base64 32` or a password manager. Never re-used across environments.**
  ...
  Set in: [07-deploy-workflow.md §7.2.2 → "Generate the secrets first"](07-deploy-workflow.md)
...
- **Backups: nightly `pg_dump` AND continuous pgBackRest WAL archiving.**
  ...
  Set in: [03-db-vm-backups.md §3.1](03-db-vm-backups.md), [03-db-vm-backups.md §3.2](03-db-vm-backups.md)
...
- **Backups replicated offsite. Tested restore within the last 90 days.**
  ...
  Set in: [03-db-vm-backups.md §3.3](03-db-vm-backups.md)
...
- **`log_min_duration_statement` is set so slow queries appear in the log.**
  ...
  Set in: [02-db-vm-setup.md §2.9](02-db-vm-setup.md), [10-troubleshooting.md §10.6](10-troubleshooting.md)
...
### 9.3 Docker and the app container
...
- **Docker Engine is from Docker's official apt repo, not Ubuntu's `docker.io` package.**
  ...
  Set in: [04-app-vm-docker.md §4.2](04-app-vm-docker.md)
...
- **Only the `deployer` user is in the `docker` group. Personal admin accounts are NOT.**
  ...
  Set in: [04-app-vm-docker.md §4.5](04-app-vm-docker.md)
...
- **Container runs as non-root (uid 1000 / node).**
  ...
  Set in: [05-app-vm-container.md §5.1](05-app-vm-container.md) (Dockerfile: `USER node`)
...
- **`read_only: true` with a `/tmp` tmpfs is set on the container.**
  ...
  Set in: [07-deploy-workflow.md §7.2.3](07-deploy-workflow.md)
...
- **Image is tagged by timestamp/commit, not just `latest`.**
  ...
  Set in: [07-deploy-workflow.md §7.3.3](07-deploy-workflow.md)
...
- **Old images pruned; disk usage trended.**
  ...
  ```bash
  $ docker system df
  # Pass: "Images" reclaimable ≪ total. If reclaimable > 5 GB, run §8.4 prune.
  $ df -h /var/lib/docker
  # Pass: "Use%" < 80%.
  ```

  Set in: [08-day-2-operations.md §8.4](08-day-2-operations.md)
...
- **TLS 1.2 + 1.3 only. No SSLv3, TLS 1.0, TLS 1.1.**
  ...
  Set in: [06-app-vm-nginx-tls.md §6.3](06-app-vm-nginx-tls.md)
...
- **Certificate renewal is monitored or automated, depending on the TLS path.**
  Why: a stalled or forgotten renewal silently breaks every login at `notAfter`. The verification differs by which §6.x cert source is in use.
  - **Let's Encrypt (§6.4)** — Certbot's snap timer should be active and a dry-run renewal must succeed:
  ...
  Set in: [06-app-vm-nginx-tls.md §6.4](06-app-vm-nginx-tls.md) (Let's Encrypt) · [§6.6](06-app-vm-nginx-tls.md#66-using-an-internal-ca-instead-of-lets-encrypt) (internal CA) · [§6.7](06-app-vm-nginx-tls.md#67-using-a-pre-purchased-commercial-certificate) (commercial / AU wildcard)
...
- **Certificate expiry monitored externally (uptime probe / cron alarm).**
  ...
  Set in: [08-day-2-operations.md §8.3](08-day-2-operations.md) (the bash health-monitor)
...
- **`/etc/greenbook.env` permissions: 640, owner root, group deployer.**
  ...
  Set in: [07-deploy-workflow.md §7.2.2](07-deploy-workflow.md)
...
- **`SESSION_SECRET` and DB password rotated within the last 12 months (or on any known exposure).**
  ...
  Set in: [07-deploy-workflow.md §7.2.2 → "SESSION_SECRET rotation (zero-downtime)"](07-deploy-workflow.md)
...
- **For higher-assurance environments: secrets moved out of `/etc/greenbook.env` into Docker secrets or a real secret manager (Vault, AWS Secrets Manager).**
  ...
  Set in: [07-deploy-workflow.md §7.2.2](07-deploy-workflow.md) ("Secrets in /etc/greenbook.env are visible to the docker group" callout).
...
- **External uptime monitor probes `https://greenbook.africanunion.org/healthz` at least every minute.**
  ...
  Set in: [08-day-2-operations.md §8.3](08-day-2-operations.md)
...
- **`/var/log/nginx/greenbook.error.log` is reviewed (or alerted on) daily.**
  ...
  Set in: [08-day-2-operations.md §8.1](08-day-2-operations.md)
